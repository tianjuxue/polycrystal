...
def generate_tess():
    ...
    os.system(f'neper -T -n {args.num_grains} -id 1 -domain "cube({args.domain_length},{args.domain_width},{args.domain_height})" -o data/neper/domain -format tess')
...
def simple():
    os.system(f'neper -T -n 20 -id 1 -o data/neper/simple -format tess')
    os.system(f'neper -T -loadtess data/neper/simple.tess -statcell facelist,npolylist -statface nfaces')
...
def show_part():
    ...
    # os.system(f'neper -V data/neper/domain.tess -showcell "y>0.5" -datacellcol real:"file(data/neper/temp)" \
